...
public class SwiftTimerQueue {
    ...
    public init(interval: DispatchTimeInterval, queue: DispatchQueue = .main) {
        ...
        internalTimer = SwiftTimer.init(interval: interval, repeats: true, queue: queue) { _ in
        }
        internalTimer.rescheduleHandler { swiftTimer in
            if self.workQueue.count>0{
                queue.async(execute: self.workQueue.first!)
            }
        }
        internalTimer.start()
    }
